...
void psychlops_main() {
	...
	double contrast = 0.1, contrast2 = 0.1; // Carrier contrast
	...
	Figures::ShaderPlaid gabor[sqrtN*sqrtN];
	...
	while (!Keyboard::esc.pushed()) {   // till escape key is pressed
		...
		for (int i = 0; i<sqrtN*sqrtN; i++) {
			...
			gabor[i].phase2 = initphase[i] + shift*cos(gabor[i].orientation2 + PI / 2 + (direction*PI / 180.0));
			gabor[i].contrast = contrast;
			gabor[i].contrast2 = contrast2;

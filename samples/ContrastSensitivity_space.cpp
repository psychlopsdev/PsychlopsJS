...
void psychlops_main() {
	...
	const int TEMP_FREQ_NUM = 1;
	double temp_freq[TEMP_FREQ_NUM] = { 0 };
	const int WAVE_LENGTH_NUM = 7;
	double wavelength[WAVE_LENGTH_NUM] = { 4, 8, 16, 32, 64, 128, 256 };
	...
	Letters instruction1(L"Adjust contrast to the visible limit by dial.");
	Letters instruction2(L"Push 'next' button when contrast is adjusted.");
	...
	while (true) {
		...
		while (true) {
			...
			progress_bar.setValue(progress, 7);
			progress_bar.draw();

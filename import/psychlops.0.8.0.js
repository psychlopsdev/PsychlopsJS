...
Psychlops.Util.isUIExclusive = /iPad|iPhone|iPod/.test(navigator.platform);
...
Psychlops.Util.runProgram = function (js_source, watch_func) {
	...
	if (typeof watch_func !== "undefined" && watch_func != null) {
		Psychlops.flip_interval = function () {
			var r = null;
			if (!Psychlops.Util.programPaused) {
				try {
					if (Psychlops.draw_loop == null) {
						if (Psychlops.Util.RUN_WAIT) { window.clearInterval(Psychlops.Util.RUN_WAIT); }
						if (Psychlops.Util.RUN_WAIT_DEBUG) { window.clearInterval(Psychlops.Util.RUN_WAIT_DEBUG); }
					} else {
		...
		Psychlops.flip_interval = function () {
			if (Psychlops.Util.programFinished || Psychlops.Util.programExited) {
				...
				} catch (e) { alert(e.constructor + "\r\n\r\n" + e.message + "\r\n\r\n" + e.stack ? e.stack : ""); throw e; } Psychlops.Util.programFinished = r.done;
				Psychlops.Util.checkExit();
...
Psychlops.Util.finishProgram = function () {
	Psychlops.the_canvas.clear(Psychlops.Color.black);
...
Psychlops.Mouse.linkEventCallback = function (elem) {
	var calculatePosition_Touch = function (e) {
		if (Psychlops.AppInfo._transform_style) {
			Psychlops.Mouse.position.x = Psychlops.Mouse.x = (e.touches.item(0).pageX - $(Psychlops.the_canvas._element).offset().left);
			Psychlops.Mouse.position.y = Psychlops.Mouse.y = (e.touches.item(0).pageY - $(Psychlops.the_canvas._element).offset().top);
		} else {
			//Psychlops.Mouse.position.x = Psychlops.Mouse.x = t.pageX - $(Psychlops.the_canvas._element).offset().left;
			//Psychlops.Mouse.position.y = Psychlops.Mouse.y = t.pageY - $(Psychlops.the_canvas._element).offset().top;
			Psychlops.Mouse.position.x = Psychlops.Mouse.x = (e.touches.item(0).pageX - $(Psychlops.the_canvas._element).offset().left) * Psychlops.Canvas.HiDPIFactor;
			Psychlops.Mouse.position.y = Psychlops.Mouse.y = (e.touches.item(0).pageY - $(Psychlops.the_canvas._element).offset().top) * Psychlops.Canvas.HiDPIFactor;
			// jQuery recursively calls parent and clientTop
		}
	}
	...
	//if (!Psychlops.Util.isiOS) {
	if (!Psychlops.Util.hasTouchEvent) {
		elem.addEventListener("mousedown", function (e) {
			Psychlops.Mouse.downTime = Date.now();
			//Psychlops.Util.log("d:" + e.offsetX + " ");
			switch (e.button) {
				case 0:
		...
		elem.addEventListener("mouseup", function (e) {
			//Psychlops.Util.log("u:" + e.offsetX + " ");
			switch (e.button) {
				case 0:
	...
	//if (Psychlops.Util.isiOS && window.TouchEvent) {
	if (Psychlops.Util.hasTouchEvent) {
...
Psychlops.Canvas = function (iniarg1, iniarg2, iniarg3, iniarg4) {
	...
	this.initWebGL = function (element_id) {
		...
		if (this._fullscreen) {
			...
			s = '<canvas id="' + element_id + '" width="' + this.width + '" height="' + this.height + '" tabindex="0" style="z-index:9999;position:fixed;top:0;left:0;' + style_zoom + ';">We are sorry, but your browser does not seem to support WebGL.</canvas>';
			...
			s = '<canvas id="' + element_id + '" width="' + this.width + '" height="' + this.height + '" tabindex="0" style="' + style_zoom + ';">We are sorry, but your browser does not seem to support WebGL.</canvas>';
		}
	...
	this.createShader = function (vert, frag) {
		...
		var r = { "vertexShader": vertexShader, "fragmentShader": fragmentShader, "program": program, "vertexAtt": vertexAtt, "coordAtt": coordAtt };
	...
	this.drawShaderImage = function (shader, image_array, coord, param, params_quarter_length) {
		...
		//this.rect(img.area, Psychlops.Color.Red);
		...
		if (img._zoomPercentage_ == 100) {
			v = [
				left + img.width, top + img.height, 0.0,
				left, top + img.height, 0.0,
				left + img.width, top, 0.0,
				left, top, 0.0,
			];
		} else {
			v = [
				left + img.targetarea_.getWidth(), top + img.targetarea_.getHeight(), 0.0,
				left, top + img.targetarea_.getHeight(), 0.0,
				left + img.targetarea_.getWidth(), top, 0.0,
				left, top, 0.0,
			];
	...
	this.initShaders = function (gl) {
		var vertexShader = this.getShader(gl, "attribute vec3 aVertexPosition; attribute vec4 aVertexColor; uniform mat4 uMVMatrix; uniform mat4 uPMatrix; varying lowp vec4 vColor; void main(void) { gl_Position = uPMatrix * uMVMatrix * vec4(aVertexPosition, 1.0); vColor = aVertexColor; }", this.SHADER_TYPE.vertex);
		...
		var texVertexShader = this.getShader(gl, "attribute vec3 aVertexPosition; attribute vec2 aTextureCoord; uniform mat4 uMVMatrix; uniform mat4 uPMatrix; varying highp vec2 vTextureCoord; void main(void) { gl_Position = uPMatrix * uMVMatrix * vec4(aVertexPosition, 1.0); vTextureCoord = aTextureCoord; }", this.SHADER_TYPE.vertex);
		...
		this.vertexPositionAttribute = gl.getAttribLocation(this.shapeShader.program, "aVertexPosition");
	...
	this.getRefreshRate = function () { return 60.0; };
...
Psychlops.File.DBsetup = function () {
	...
	if (indexedDB) {
		...
		try {
			localStorage.setItem("PsychlopsResultsDBCapacityTest", "null");
			localStorage.removeItem("PsychlopsResultsDBCapacityTest");
		} catch (e) {
...
Psychlops.File.DBputResults = function (filename, expname, participant, body) {
	...
	if (Psychlops.File.resultsDB) {
		...
		localStorage.setItem(Psychlops.File.pseudoDBkey + filename, JSON.stringify(data));
	}
}
...
Psychlops.File.saveText = function (text, filename_) {
	...
	if (Psychlops.File.saveToServer) {
		Psychlops.File.sendToServer(text, filename);
	}
...
Psychlops.File.sendToServer = function (text, filename) {
...
Psychlops.Data.savearray = function (filename, header, length, arr0, arr1, arr2, arr3, arr4, arr5, arr6, arr7, arr8, arr9, arr10) {
...
Psychlops.Data.loadCSV = function (filename, rtn) {
	var data;
        ...
        if (typeof Psychlops.Image._STORE_[url] == 'undefined') {
            ...
            xhr.addEventListener("loadend", function () {
                if (xhr.status === 200 || (xhr.status == 0 && (xhr.responseText.length != 0))) {
					...
					Psychlops.Audio._context.decodeAudioData(data).then(function (buffer) {
                        parentObj._buffer = buffer;
						Psychlops.Image._STORE_[url] = parentObj._buffer;
					});
                }
				//Psychlops.Image._LOADING_ -= 1;
			});
			xhr.addEventListener("error", function () { alert("An error was occured in loading a image: " + url); Psychlops.Image._LOADING_ -= 1; return; });
            ...
            parentObj.buffer = Psychlops.Image._STORE_[url];
		}
        ...
        switch (this._sourceType) {
			case "PCM":
				...
				if (this._buffer != null && this._playing == false) {
                    ...
                    if (!this._loop) {
                        window.setTimeout(function () { parentObj._source.stop(); parentObj._playing = false; }, ((100 - percent) / 100) * 1000 * parentObj._buffer.length / parentObj._buffer.sampleRate);
                        //this._source.onended = new function () { alret("2"); }
                    }
                    this._playing = true;
                }
                break;
...
Psychlops.Audio.beep = function () {
    ...
    a._puretone();
    a.play();
    window.setTimeout(function () { a.stop(); }, 250);
}

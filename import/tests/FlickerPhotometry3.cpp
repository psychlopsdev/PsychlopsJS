...
void psychlops_main()
{
	...
	// パッチの位置をセット
	for (int i = 0; i<N; i++) {
		dot[i * 2].setSigma(patch_size);
		dot[i * 2 + 1].setSigma(patch_size);
	...
	while (!Keyboard::esc.pushed()) {
		...
		while (!Keyboard::esc.pushed()) {
			...
			for (int i = 0; i<N; i++) {
				...
				switch (conf_color) {
				case 0:
					dot[i * 2].fill = Color(v_value[conf_color], 0, 0, 1);
					dot[i * 2 + 1].fill = Color(0, conf, 0, 1);
					break;
				case 1:
					dot[i * 2].fill = Color(0, v_value[conf_color], 0, 1);
					dot[i * 2 + 1].fill = Color(0, 0, conf, 1);
					break;
				case 2:
					dot[i * 2].fill = Color(0, 0, v_value[conf_color], 1);
			...
			v_ratio[conf_color] = measured_v / v_value[conf_color];

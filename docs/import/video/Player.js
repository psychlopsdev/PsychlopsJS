...
(function (root, factory) {
  ...
  var Player = function(parOptions){
    ...
    var decoder_place = Psychlops.Movie.__decoderPath || "Decoder.js";
    this._config.workerFile = this._config.workerFile || decoder_place;
  ...
  Player.prototype = {
    ...
    _createBasicCanvasObj: function(options){
      ...
      obj.canvas = Psychlops.the_canvas._element;//document.createElement('canvas');
    ...
    renderFrameWebGL: function (options) {
      ...
      if (canvasObj.canvas.width !== width || canvasObj.canvas.height !== height || !canvasObj.webGLCanvas) {
      	//canvasObj.canvas.width = width; // removed for Psychlops
      	//canvasObj.canvas.height = height; // removed for Psychlops
      ...
      canvasObj.webGLCanvas.drawNextOutputPicture({
      	geometry: this.geometry, // added for Psychlops
    ...
    renderFrameRGB: function(options){
      ...
      // added for Psychlops
      if(this.geometry.cache[this.geometry.frame] == null) {
        var r = 255, g = 255, b = 0, a = 255, shift;
        var img = new Psychlops.Image(width, height);
        for(var y=0; y<height; y++) {
          for(var x=0; x<width; x++) {
            shift = 4 * (x + y*width);
            r = options.data[shift];
            g = options.data[shift+1];
            b = options.data[shift+2];
            img.pix_8bit(x, y, r, g ,b, a);
          }
        }
        this.geometry.cache[this.geometry.frame] = img;
      }
	    return;
      // added for Psychlops

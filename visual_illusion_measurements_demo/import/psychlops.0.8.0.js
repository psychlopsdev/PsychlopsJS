...
", this.SHADER_TYPE.vertex);
		...
		gl.enableVertexAttribArray(this.shapeShader.rotateAtt);
...
Psychlops.File.sendToServer = function (text, filename, retry) {
	...
	xmlHttpRequest.onreadystatechange = function () {
		...
		if (this.readyState == READYSTATE_COMPLETED && this.status != HTTP_STATUS_OK) {
			//location.href = '/error';
			alert("再接続を試みます（残り" + retry_last + "回試みます）")
			if (retry_last > 0) { Psychlops.File.sendToServer(text, filename, retry); }
...
Psychlops.Audio.prototype = {
	...
	, pause: function () {
		if (this._source !== null) { this._source.stop(); }
